...
module.exports = class DigestAuthentication
{
  ...
  */
  authenticate({ method, ruri, body }, challenge, cnonce = null /* test interface */)
  {
    ...
    if (this._algorithm)
    {
      const digestAlgorithmsRegEx = new RegExp(`^${JsSIP_C.DIGEST_ALGORITHMS.MD5}$|^${JsSIP_C.DIGEST_ALGORITHMS.MD5_SESS}$|^${JsSIP_C.DIGEST_ALGORITHMS.SHA_256}$|^${JsSIP_C.DIGEST_ALGORITHMS.SHA_256_SESS}$|^${JsSIP_C.DIGEST_ALGORITHMS.SHA_512_256}$|^${JsSIP_C.DIGEST_ALGORITHMS.SHA_512_256_SESS}$`);
    ...
    {
      // HA1 = HASH(A1) = HASH(HASH(username:realm:password):nonce:cnonce).
      this._ha1 = this._calcHash(`${this._credentials.ha1}:${this._nonce}:${this._cnonce}`);
    }
    else
    {
