...
const { DIGEST_ALGORITHMS } = require('./Constants');
...
const checks = {
  ...
  optional : {
    ...
    {
      const _supportedDigestAlgorithms = [];

      if (Array.isArray(supported_digest_algorithms) &&
        supported_digest_algorithms.length)
      {
        for (const algorithm of supported_digest_algorithms)
        {
          if (algorithm === DIGEST_ALGORITHMS.MD5 ||
            algorithm === DIGEST_ALGORITHMS.MD5_SESS ||
            algorithm === DIGEST_ALGORITHMS.SHA_256 ||
            algorithm === DIGEST_ALGORITHMS.SHA_256_SESS ||
            algorithm === DIGEST_ALGORITHMS.SHA_512_256 ||
            algorithm === DIGEST_ALGORITHMS.SHA_512_256_SESS)
          {
